...
FaceDetectionResult mtcnn::Detect(const Mat &image, std::vector<FaceInfo> &faces){
    ...
    for(vector<struct Bbox>::iterator it=thirdBbox_.begin(); it!=thirdBbox_.end();it++){
        if((*it).exist){
            ...
            faces.push_back(fi);
            cv::rectangle(image, cv::Point((*it).y1, (*it).x1), cv::Point((*it).y2, (*it).x2), Scalar(255,0,0), 2,8,0);
            for(int num=0;num<5;num++)
                circle(image,cv::Point((int)*(it->ppoint+num), (int)*(it->ppoint+num+5)),3,Scalar(0,0,255), -1);
        }
    }
    firstBbox_.clear();
