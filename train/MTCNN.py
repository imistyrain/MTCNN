...
if __name__ == '__main__':
    ...
    model_dir = root_dir+"/model/pytorch"
    parser = argparse.ArgumentParser(description='MTCNN Demo')
    parser.add_argument("--test_image", default=root_dir+"images/test.jpg")
    parser.add_argument('--mini_face', default="20", type=int)
    args = parser.parse_args()

    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    image = cv2.imread(args.test_image)
    start = time.time()
    bboxes, landmarks = create_mtcnn_net(image, args.mini_face, device, p_model_path=model_dir+'/pnet_Weights', r_model_path=model_dir+'/rnet_Weights', o_model_path=model_dir+'/onet_Weights')
    ...
    if landmarks is not None:
        for i in range(landmarks.shape[0]):
            ...
            for j in range(5):
                cv2.circle(image, (int(landmark[j, 0]), int(landmark[j, 1])), 3, (0, 0, 255), -1)
