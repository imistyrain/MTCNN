...
train_path = '../data_preprocessing/anno_store/imglist_anno_48.txt'
val_path = '../data_preprocessing/anno_store/imglist_anno_48.txt'
batch_size = 512
...
for epoch in range(num_epochs):
    ...
    for phase in ['train', 'val']:
        ...
        for i_batch, sample_batched in enumerate(tqdm(dataloaders[phase])):
            ...
            input_images, gt_label, gt_offset, landmark_offset = sample_batched['input_img'], sample_batched[
            ...
            # print('lankmark_offset shape is ',landmark_offset.shape)
            ...
            with torch.set_grad_enabled(phase == 'train'):
                ...
                if len(valid_landmark_offset) != 0:
                    loss += 3*loss_landmark(valid_pred_landmark, valid_landmark_offset)
        ...
        epoch_loss_landmark = running_loss_landmark / dataset_sizes[phase]
        epoch_accuracy = running_correct / (running_gt + 1e-16)

        print('{} Loss: {:.4f} accuracy: {:.4f} cls Loss: {:.4f} offset Loss: {:.4f} landmark Loss: {:.4f}'
              .format(phase, epoch_loss, epoch_accuracy, epoch_loss_cls, epoch_loss_offset, epoch_loss_landmark))
